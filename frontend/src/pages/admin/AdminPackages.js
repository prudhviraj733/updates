...
const EMPTY = { name: "", description: "", image_url: "", package_type: "bundle", product_ids: [], price: 0, is_active: true, location_ids: [] };
          ...
          <div className="grid gap-3">
            ...
            <div><Label>Products</Label>
              <div className="mt-1 max-h-40 space-y-1 overflow-y-auto rounded border p-2">
                {products.map((p) => <label key={p.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.product_ids.includes(p.id)} onChange={() => toggleProduct(p.id)} />{p.name}</label>)}
              </div>
            </div>
