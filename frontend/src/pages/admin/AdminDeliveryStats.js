...
export default function AdminDeliveryStats() {
  ...
  const totals = pins.reduce((a, p) => ({ orders: a.orders + p.orders, sales: a.sales + p.sales, asap: a.asap + p.asap_orders }), { orders: 0, sales: 0, asap: 0 });
  ...
  return (
    <div data-testid="admin-delivery-stats">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border bg-white p-4"><p className="text-xs uppercase text-slate-400">ASAP Orders</p><p className="mt-1 text-2xl font-bold text-saffron">{totals.asap}</p></div>
      </div>

      <div className="mt-6 overflow-x-auto rounded-xl border bg-white">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-slate-500"><tr><th className="p-3">PIN</th><th className="p-3">Orders</th><th className="p-3">Sales</th><th className="p-3">AOV</th><th className="p-3">Customers</th><th className="p-3">ASAP</th><th className="p-3">Serviceable</th></tr></thead>
          <tbody>
            {pins.map((p) => (
              <tr key={p.pincode} className="border-t" data-testid={`pin-stat-${p.pincode}`}>
                ...
                <td className="p-3">{p.asap_orders}</td>
