## 2026-08-29 — Order-level Profit Calculation & Profit Analytics (ADMIN ONLY, extended not rebuilt)
- **Scope/decisions (user-confirmed)**: profit is computed for the COMPLETE order (never per-product as the primary metric). Purchase cost for an order = the immutable snapshotted `item.cost_price` (batch `purchase_price` is stored ONLY for inventory valuation, not order profit). Margin denominator = **Net Product Revenue only**. **GST excluded** from revenue/profit (collected tax, shown separately). Delivery cost = per-PIN default (`pincode.delivery_cost`) auto-filled on the order + editable per order. Realized profit = status `delivered` OR payment_status `paid`/`refunded`, NOT cancelled; refunds reduce revenue/profit in period aggregates.
- **Formula (per order, ex-GST)**: GrossProductSales(Σ mrp×qty) − ProductDiscounts(product_discount+combo_discount) − CouponDiscounts = NetProductRevenue; − PurchaseCost(Σ cost_price×qty) = GrossProductProfit; + DeliveryCollected(delivery_charge+express_charge−delivery_discount) − ActualDeliveryCost(order.delivery_cost) = OrderNetProfit; Margin% = OrderNetProfit/NetProductRevenue×100. Inclusive-GST orders strip embedded `gst.total_tax` out of NetProductRevenue; exclusive prices already ex-GST.
- **Backend NEW `routers/profit.py`**: `compute_order_profit()` (reads only immutable order snapshots), `is_realized()`, `_refunds_by_order()` (wallet_ledger reason=refund + Razorpay return refunds), `_resolve_range()` (today/yesterday/week/month/prev_month/custom, UTC). Endpoints: `GET /admin/orders/{id}/profit`, `PUT /admin/orders/{id}/delivery-cost` (delivery_cost internal; delivery_charge edit adjusts final_amount by delta), `GET /admin/profit/summary` (realized vs in_progress totals + daily[] + cancelled count), `GET /admin/profit/breakdown` (by product/category/subcategory/subsubcategory), `GET /admin/profit/coupons` (public+private coupon profit impact), `GET /admin/profit/inventory` (value at cost using batch purchase_price→cost_price fallback, clamped to available; potential sales/profit).
- **Models**: `InventoryBatchInput.purchase_price`, `PinCodeInput.delivery_cost`, `OrderDeliveryCostInput`. `orders.create_order` snapshots `delivery_cost` from the PIN. `inventory.add_batch` stores per-batch `purchase_price` (never overwrites older batches; LEGACY batch price=null).
- **SECURITY**: `orders._scrub_internal()` removes `cost_price`/`delivery_cost`/`batch_allocations` from `GET /orders` and `GET /orders/{id}` for non-admins (admin keeps them). All `/admin/profit/*` are require_admin (401/403 for customers). Customer PDFs unchanged (no cost/profit exposed).
- **Admin UI**: NEW `AdminProfit.js` (`/admin/profit`, nav "Profit & Margins") — 11 KPI cards, daily Sales-vs-Profit bar chart + margin line chart (recharts), daily table, 4 tabs (Overview / Products & Categories / Coupon Impact / Inventory Value), 6 period buttons + custom date range (empty-state + error/retry). `AdminOrderDetail.js` adds a "Profit (internal)" waterfall card + editable delivery charge/cost. `AdminInventory.js` add-batch dialog gains Purchase price/unit + batch table column. `AdminPinCodes.js` gains "Actual delivery cost" per-PIN field.
- **Verified**: testing_agent iteration_28 — backend 100% (47 passed/1 skipped), frontend 100% of tested flows. Post-test fixes applied: DOM-nesting (p→div) in order profit card, custom-range empty/error state, inventory cost clamp for reserved units, "Stock rows" label + breakdown gross-margin note. Refund-subtraction path self-verified (temp ₹50 refund → summary refunds 50, net_profit −50, cleaned up). All modified real/synthetic data restored. NOT user-confirmed. NOT deployed.


## 2026-08-28 — Inventory batches + expiry + low-stock + TRUE FEFO (per-PIN, extended not rebuilt)
- **No migration / legacy-safe**: each inventory row gains optional `batches: [{id, batch_number, quantity, expiry_date, added_at}]`. `available_quantity` stays the authoritative concurrency gate; batches are the consumed ledger. Legacy rows (no batches) keep working; when the first batch is added to a row with legacy stock, that remainder is folded into a `LEGACY` batch so batches always sum to available (prevents Save/ledger drift).
- **Backend** (`routers/inventory.py`, `models.py`, `routers/orders.py`): `InventoryBatchInput`; `GET /admin/inventory` returns batches + per-batch and row-nearest expiry status (🟢 normal / 🟡 ≤30d / 🔴 ≤7d / ⚫ expired) + "Expires in N days"; `POST /admin/inventory/batch` (additive, folds legacy), `DELETE /admin/inventory/batch/{id}`, `GET /admin/inventory/expiry-summary` (active-products only: total/low/out/exp30/exp7/expired). `PUT /admin/inventory` recomputes available from batch sum for batched rows. Per-row `low_stock_threshold` configurable. **FEFO**: order reservation keeps the atomic `available>=qty` gate, then decrements batches earliest-expiry-first (per-batch `quantity>=take` guard), records `batch_allocations` on the order; cancel restores available + reserved + exact batch quantities (re-pushing pulled batches with original expiry).
- **Admin UI** (`AdminInventory.js`): clickable dashboard summary cards → status filter; "+ Batch" dialog (batch no/qty/expiry); expandable per-batch rows (Product|Batch|Qty|Expiry|Expiry Status|Stock Status); filters (all/enabled/disabled/out/low/expiring30/expiring7/expired) + sort (name/nearest expiry/lowest stock/out first) + batch-number search; controlled Available/threshold inputs (Available read-only for batched rows); badges ⚫ Out / 🔴 Low / 🟢 In Stock.
- **Verified**: testing_agent iterations 26 (backend 19/19 pytest incl. real-order FEFO + restore-on-cancel + legacy) & 27 (frontend fixes confirmed). Post-27 mixed-row legacy-loss HIGH bug fixed and self-verified via exact repro (55→62→Save 62→delete→55). Test data cleaned; Almonds row holds a LEGACY batch (available 55, correct).
...
- **Backend** (`routers/coupons.py`): `/coupons/available` excludes private + logs impressions; `/coupons/validate` wrapped to log apply success/failure (private still validates by code but must pass all eligibility/targeting/first-order/usage rules). Fixed `usage_limit==0` now blocks correctly (`is not None`). New admin endpoints: `GET /admin/coupons/analytics` (dashboard totals + per-coupon rows + expiring), `GET /admin/coupons/{id}/stats` (funnel Viewed→Applied→Validated→Redeemed, conversion rates, unique/first-time-customer/repeat, usage%/remaining), `GET /admin/coupons/{id}/usage` (admin-only history). Date ranges: today/7d/30d/month/all/custom. `first_time_users` = customers whose first-ever order used the coupon.
- **Admin UI**: `AdminCoupons` gains visibility toggle + badge, dashboard summary cards, filters (range/visibility/status/used/category), per-card mini stats + "View analytics" button, empty state, dialog a11y descriptions. New `AdminCouponStats` page (`/admin/coupons/:id/stats`) with funnel, metrics, range buttons, and usage table.
- **Customer UI**: no visible change except private coupons no longer appear in the Available Coupons modal.
- **Verified**: testing_agent iteration_25 — backend 39/39 pytest, frontend 100% (dashboard, filters, private badge, stats page, private-by-code apply, private hidden from modal). Post-review fixes (usage_limit=0, first_time semantics, view de-dup+indexes, a11y, empty state) applied and re-verified. All test data cleaned up.


## 2026-08-28 — Sub-subcategory system (Category → Subcategory → Sub-subcategory → Product)
- **Data (no migration)**: sub-subcategory = a `categories` doc whose `parent_id` = a subcategory id (which itself has parent_id = a top category). `ProductInput.subsubcategory_id` optional; legacy products (null) unaffected.
- **Backend** (`routers/categories.py`, `routers/products.py`): new `GET /api/subsubcategories[?subcategory_id=]` + `GET /api/admin/subsubcategories`; level-guarded listing (wrong-level parent id returns `[]`, no cross-level leak); `_validate_hierarchy` on product create/update enforces the exact category→subcategory→sub-subcategory chain server-side (400 on mismatch, IDs authoritative); `GET /api/products?subsubcategory_id=` filter; new `GET /api/products/search-resolve?q=&location_id=&pincode=` distinguishing `not_found` vs `out_of_stock` and returning available alternatives (priority sub-subcategory→subcategory→category, excludes the item). Combo swap priority (`routers/packages.py`) upgraded to subsub→sub→cat. Duplicate-name error now depth-labeled ("Sub-subcategory …").
- **Admin web**: new `AdminSubSubcategories` page (`/admin/subsubcategories`, dependent Category→Subcategory selectors, reuses `/admin/categories` CRUD); AdminProducts gained dependent category→subcategory→sub-subcategory selectors, a 3-level filter bar, hierarchy breadcrumb per row, and inactive records excluded from selects.
- **Customer web** (`Products.js`): strict drill-down — a subcategory WITH sub-subcategories shows a chip band + "Pick a type" prompt and no products until a chip is picked; an **"All &lt;subcategory&gt;" chip** (sentinel `subsubcategory=all`) keeps legacy null-subsub products reachable; subcategories with none keep legacy display; breadcrumbs Home→Category→Subcategory→Sub-subcategory with up-navigation; search uses search-resolve (out-of-stock banner + addable alternatives, not-found message). Added catch-all route (`*`→`/`) so unknown paths (e.g. `/cart`) no longer render blank.
- **Mobile** (`ProductsScreen.js`): mirrors category→subcategory→sub-subcategory chips (incl. "All" chip), breadcrumb, and search-resolve alternatives. Source-level only; not device-tested.
- **Verified**: testing_agent iteration_24 — backend 100%, frontend 100% (all 6 iteration_23 defects fixed: cross-level leak, legacy reachability, blank /cart, duplicate label, inactive-in-selects, table hierarchy). Local suite `backend/tests/test_subsubcategory.py` passes. Leftover TEST_PROD data purged. Mobile NOT device-verified.
...
- **Receipt content**: SavingSmart branding header, invoice/receipt number (INV-/WRC-/RFD-), order/txn id, date-time, customer + delivery address, itemised products/qty/rate/amount (incl. combo lines), subtotal, MRP/combo/coupon discounts, delivery + 30-min charges, delivery coupon, wallet used, amount payable, payment method + **actual status** (COD unpaid shows "Pending" + note, never "Paid"), Razorpay payment/order id, refund amount/method/reference, wallet direction/balance-after. No GST invented (shows "inclusive of taxes" unless a GST number is configured).
- **Frontend**: `lib/receipt.js` (blob download/print via auth api client); buttons in customer Orders detail (Download + Print), Wallet ledger rows (Receipt), refund rows (Refund receipt), and admin Order detail (Receipt + Print).
- **Security**: customer 403 on another customer's receipt; 401 unauthenticated; 404 unknown id; admin can access any order/return receipt.
- **Verified**: backend/tests/test_receipts.py + test_refund_receipt.py (COD, online, discounted, wallet, refund, 403/404) all pass; PDFs rendered & visually confirmed (order totals reconcile 1398−302 MRP−139.80 coupon+60 delivery = 1318.20; refund receipt clean). testing_agent iteration_22: frontend 5/5 receipt flows + security pass (refund UI link only unexercised because no returns exist in this DB; endpoint verified via synthetic data).

## 2026-08-26 — Coupon system enhancement (visibility/eligibility split, first-order, usage limits, modal UI)
- **Model**: CouponInput gained `first_order_only` (bool) alongside `category_id`; `usage_limit`/`usage_limit_per_customer` now enforced. No migration — existing coupons default to whole-cart, all-customers, unlimited via `.get()`.
- **Visibility vs application eligibility split** in `GET /api/coupons/available`: VISIBILITY hides a coupon entirely (inactive, out of date window, wrong location/PIN, target-user mismatch, first-order-only for returning customers, global/per-customer usage limit reached). APPLICATION eligibility keeps it visible but flags `eligible`/`reason`/`shortfall`/`state` (available|almost|condition) when the cart doesn't yet meet the min. First-order status is server-derived (zero non-cancelled orders); returning customers never see a "not eligible" message for first-order coupons.
- **Server-side security**: `/coupons/available` and `/coupons/validate` now derive cart total + per-category subtotals SERVER-SIDE (`_cart_breakdown`) and IGNORE the client `subtotal` param (tamper-proof). `validate` + order creation call `_assert_eligible` (validity, target, first-order, usage limits) and enforce category-only minimums from the DB cart / order snapshot. `_bulk_usage_counts` computes redemptions in one pass (perf).
- **Customer UI** (Checkout.js): replaced the inline coupon list with a "View available coupons" button opening a modal (`coupons-modal`) showing per-coupon states — Apply (eligible), "Add ₹X more of <Category> products" + Add More→/products?category=<id> (almost), or a clear condition message; applied coupons show "Applied · Remove". Modal refetches after apply/remove.
- **Admin UI** (AdminCoupons.js): Eligible-category selector, Customer-eligibility (first-order-only) toggle, total + per-customer usage-limit inputs; cards show category / First-order / usage-limit badges.
- **Verified**: testing_agent iteration_21 — backend 22/22 pytest, frontend 100% of tested flows (general + category modal apply/remove/add-more, first-order hidden from returning customer, usage limits, stacked product+delivery COD order e2e, admin CRUD). Regression: existing cart-wide + delivery coupons unchanged. Tests: backend/tests/test_coupon_edge_cases.py, test_category_coupon*.py, test_general_tamper.py, test_coupon_iter22.py.
